...
def test_tick_requires_psi():
    clock = ClockRateModulator()
    with pytest.raises(ValueError):
        clock.tick(psi=None, wall_delta=1.0)


def test_strict_mode_rejects_out_of_range_psi():
    clock = ClockRateModulator(strict_psi_bounds=True)
    with pytest.raises(ValueError, match=r"within \[0, 1\]"):
        clock.tick(psi=1.5, wall_delta=1.0)


def test_non_strict_clamps_out_of_range_psi():
    clock = ClockRateModulator(strict_psi_bounds=False)
